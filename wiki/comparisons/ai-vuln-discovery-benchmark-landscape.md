...
Each benchmark targets a different rung of the discovery-to-action pipeline, so the set is complementary.
...
Offense deepens left-to-right, from reproducing a bug through developing an exploit to monetizing it; CTI-REALM is the defender-side mirror, turning intelligence into deployed detections. [[xbow-mythos-evaluation|XBOW]]'s private *StorageDrive* web-exploit benchmark and [[mdash|MDASH]]'s harness-on-CyberGym result sit alongside as vendor-run surfaces.
