...
Two September 2026 announcements supply further vendor examples against rungs this ladder already grades, and move none of them. CrowdStrike states that its [[crowdstrike-agentic-identity-provider|Agentic Identity Provider]] issues a cryptographically verifiable per-agent identity, brokers short-lived tokens scoped to one task in place of standing credentials, and binds every action to the delegating human or workload — the L3 attested-identity and human-owner criteria, plus the L4 zero-credentials and task-bound-token criteria. CrowdStrike also states the product is in development, gives no general-availability date, and publishes no token format, key derivation, or revocation model, so it reaches no part of the L5+ criterion. Ping states that [[ping-enterprise-personal-agent-access|Enterprise Personal Agent Access]] discovers personal agents including shadow AI, issues secretless just-in-time privileged access, and revokes that access in real time, which is the L4 zero-credentials criterion and the L5 shadow-agent-discovery criterion. Ping states it is available now. Neither vendor publishes a coverage figure, a bypass rate, or an independent evaluation, so the graded status of every rung named here is unchanged.
