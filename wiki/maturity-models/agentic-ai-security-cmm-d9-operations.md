...
Companion deep-dive to [[agentic-ai-security-cmm-2026|the CMM]]'s D9 domain, written under the [[agentic-ai-security-cmm-recalibration-method-2026|recalibration method]]. D9 is the most process- and labor-heavy domain in the model, and the least product-driven. About half of it has no product answer on any platform and is pure operating-model work: HITL-fatigue and oversight-quality measurement, IR-runbook authoring, decommission drills, bus-factor continuity. Where products exist, they are platform-native primitives repurposed from adjacent domains (D2 identity lifecycle, D7 observability) rather than built as D9-specific tools.
...
- **The oversight path exercised adversarially**, with the testing programme covering urgency-driven bypass, approval-fatigue sequences, and multi-step normalisation ahead of a critical action, session-level approval rate limits enforced, and a session flagged where its approval rate significantly exceeds its own historical baseline.[^aix-oversight]
