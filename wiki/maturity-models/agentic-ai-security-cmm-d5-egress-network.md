...
- **L5 — Optimizing.** A mesh-deployed agent-aware proxy runs per agent with zero bypass; per-task egress capability tokens bind to the specific upstream resource; SSRF and direct-egress paths are closed at the network layer so the gateway is the only path out, which includes the allowlisted internal services an agent may still call — each is itself egress-constrained or it is a relay; the A2A signing profile is published and audited per release; the MCP CVE feed is wired to auto-quarantine without HITL.
...
**A coding tool egresses to source control and the language server, and nowhere else** — until it calls an MCP tool, at which point the agent-aware gateway becomes load-bearing.
...
D5 is the domain where removing the egress capability most directly defeats the [[lethal-trifecta|lethal-trifecta]]: a bot with no sensitive data or no egress path drops the D5 requirement substantially. Confirm the removed leg is genuinely absent, not merely policy-restricted.
