...
verified_against:
  ...
  - ".raw/papers/owasp-ai-exchange-testing-2026-08-19.md"
verified_findings: 0
verified_note: "Verify-and-fix for the #170 harness-PDP decision. Read LEAST MODEL PRIVILEGE and the Exchange testing doc in full against L3. The premise reversal holds: testing doc line 91 pairs the direct-gateway test with an injection rationale and states no other property, and oversight-layer.md:179 defines the phrase as injection resistance. Fixed: the line-93 restatement is accurate to the source, so the departure it leaves is now stated on the page (the Exchange enumerates gateway, mesh and tool-execution proxy and names no hosting runtime); the protocol-pointer sentence claimed three substitutes where the protocol names four; the gaps-item parenthetical was process commentary."
---
...
- **A synchronous, fail-closed gate.** No action proceeds until a permit or deny returns, and an unreachable policy engine denies.[^aix-leastmodelpriv] Both properties are read off the PDP's deployed configuration, and a deployment whose agents proceed on policy-engine timeout has not met this rung whatever its policy language. Where the policy engine runs in-process, the same reading is taken on its policy input, and an enforcement point that proceeds when its policy source is absent or malformed has not met the rung either.
...
**Scoping a grant at issue time bounds third-party blast radius.** An MCP or skill provider is consumed by callers it does not control, so its selective L5 target scopes each grant when it issues it rather than trusting the caller at call time. The holder-bound per-task form of that scoping sits a rung above the row, at L5+.
