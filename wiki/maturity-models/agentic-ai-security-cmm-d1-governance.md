...
- **L3 — Defined.**
    ...
    Every criterion yields a document or a configuration record rather than a product, so the level is graded off the register, the minutes and the resolved settings.
- **L4 — Managed.**
...
[[owasp-state-of-agentic-ai-security-governance|OWASP's State of Agentic AI Security and Governance]] supplies parallel governance-maturity levels (Levels 0–4, from unaware/ad-hoc through experimentation, policy-defined HITL, integrated continuous oversight, to adaptive self-regulation) that this domain's levels track closely. Its central finding frames D1: organizations are deploying agents faster than they can govern them, and additional budget for existing programs does not close that gap. The report pairs the governance levels with an Adoption Tier (AT0–AT8) so required governance scales with what is deployed rather than against a flat checklist — the same right-sizing logic the deployment-shape table below applies.

## Assessor detail per level

L1 describes where a deployment starts and carries no criterion, so a deployment that does not meet every L2 criterion scores L1, or 0 under the measurement protocol's rubric where no evidence of the L1 baseline exists. L2, L4, and L5+ are graded from their statements above. The two levels detailed below carry criteria an assessor checks item by item, each list stating what its own level adds. For three deployment shapes, [[#L3 evidence by deployment shape]] states how an L3 criterion resolves and what further evidence an L3 claim needs.
...
| Desktop-agent productivity assistant ([[claude-cowork\|Claude Cowork]] class) | L3 | Enterprise custom roles grant each capability from a closed default, which is the decision-rights record this level asks for per agent type |
...
- *"L5 is unreachable because it names a just-GA'd certification we can't procure in time."* Addressed: L5 is scheme-neutral, and the production-maturity qualifier in the preamble to the level definitions applies. A scheme in the organization's approved-vendor pipeline satisfies the assurance criterion from the documented date its assurance took effect, and a scheme still in procurement satisfies nothing until then.
