...
- Bounded by: [[agentic-ai-security-ra-gaps|Agentic AI Security RA Gaps]] — twelve items, eleven of them a control no product ships or a property no vendor documents, and the twelfth a scope decision no plane table yet enforces. Where the gap is an unshipped control, it caps the level a domain can reach on evidence, because an assessor cannot collect an artifact nobody ships.
