...
verified_against: []
verified_findings: 1
verified_note: "D1 mapping paragraph fixed: lock rows added to the first half, step 6 named as a method step, strict-customization lock covers user scope. Open: Settings-key locations text predates the vendor's managedSourcesBehavior merge mode and four-source managed tier."
---
...
The practice applies whenever an agentic coding harness reads a repository. Which controls are load-bearing depends on which of the five shapes in [[generative-coding-deployment-shape-2026|Generative Coding Deployment Shapes]] is in play; read that page first if the deployment is not obviously interactive.
...
**`RA capability → CMM`** anchors the row to the [[agentic-ai-security-reference-architecture|AAI-S RA]] capability it implements and the [[agentic-ai-security-cmm-2026|CMM]] domain that scores it. The catalog's rows and the RA's capability rows differ in scope and grain. The RA lists technology-neutral capabilities for agentic systems in general; this catalog lists controls for one deployment shape, at the granularity of the person configuring a harness. Some rows map onto an RA capability exactly, some map onto one at a coarser granularity, and nine map onto no RA row at all. Those nine carry a dash in that column and are analyzed in [Controls with no RA capability](#controls-with-no-ra-capability).
...
The `permissions.ask` row keeps firing under an autonomous mode, and its behavior is specific enough to test. A bare `Bash` or `Bash(*)` ask rule is *skipped* for any command that runs sandboxed, so an organization that wrote one and then enabled the sandbox has no prompt left. An ask rule scoped to an argument pattern still fires. An ask rule on `Bash(dangerouslyDisableSandbox:true)` catches the escape hatch in flight.

The merge semantics in [Settings-key locations](#settings-key-locations) decide the score: `excludedCommands` merges across scopes and has no managed-only lockdown, so a developer can always append entries that run commands outside the sandbox. Keep the managed list narrow and treat it as a reviewed artifact.

**Eight managed-settings keys and one deployed configuration file admit MCP servers and close the customization escapes.** Four keys and the file govern admission:

- `allowedMcpServers`, the server allowlist, is taken whole from the highest-ranked source that sets it, and while malformed it is enforced as an **empty** allowlist.
- `deniedMcpServers`, the server denylist, merges from every scope.
- `allowManagedMcpServersOnly`, together with `managedMcpServers` and the deployed `managed-mcp.json`, pins the admitted set to what a managed configuration names.

The remaining four keys lock the customization sources:

- `allowManagedPermissionRulesOnly` extends the same managed-only semantics to permission rules.
- `disableBypassPermissionsMode` disables the permissions-bypass mode.
- `disableSideloadFlags` rejects the plugin-directory, plugin-URL, agents and MCP-config startup flags outright.
- `strictPluginOnlyCustomization`, the strict-customization lock, blocks skills, agents, hooks and MCP servers loaded from user or project sources, and leaves plugin-delivered customization as the only route.

**These rows are this shape's policy decision point, and [[agentic-ai-security-cmm-d3-control-least-agency|D3]] L3 grades them as one.** The harness resolves the managed policy and returns permit or deny before the tool call runs, and the settings files the policy arrives in are write-denied to the session, so the decision holds against an injected instruction. That write-deny reaches two paths unevenly: the sandbox covers shell commands and their children, and the file-editing tools run under the permission system instead, so a deployment resting on the sandbox alone leaves the second path to a permission rule it has to write. What changes is the evidence set. An external decision point is tested through the interface it exposes; here the assessment collects five records:

- the resolved settings read from an enrolled device, with the delivery route named
- the write-deny over the settings files and the managed directory, with the rule covering the file-editing path beside it
- the `ConfigChange` record of in-session changes
- a `permissions.deny` rule firing under the most permissive autonomy mode the deployment allows
- the empty allowlist a malformed `allowedMcpServers` produces

The write-deny is a sandbox property and is lost where `filesystem.disabled` switches the sandbox's filesystem layer off, so the resolved value of that key belongs in the same record. [[agentic-ai-security-cmm-measurement-protocol|The measurement protocol]] states the two limits on the substitution, and the circularity it records: vendor code interprets the customer's policy file.
...
An assessor must verify four configuration facts rather than assume them:

- **Read access.** Sandbox read access defaults to the whole filesystem minus explicit denies, so `~/.aws` and `~/.ssh` remain readable until `sandbox.credentials` or `denyRead` entries exist.
- **Escape hatch.** The escape hatch is a tool parameter, `dangerouslyDisableSandbox`, which the model can set to retry a command that failed under isolation. Setting `allowUnsandboxedCommands` makes the harness ignore the parameter entirely, which the `/sandbox` panel labels strict sandbox mode.
- **Bash only.** The built-in sandbox covers Bash and its child processes only. Read, Edit, and Write run under the permission system instead, which is how the [[claude-code-github-action-credential-exposure|Microsoft finding]] reached `/proc/self/environ` while the Bash boundary held.
- **Filesystem layer off.** `filesystem.disabled` (v2.1.216 and later) is newer than the rest of this catalog and inverts a control an assessor may already have scored. The key turns the filesystem layer off while leaving network isolation on. Setting it also disables the `sandbox.credentials.files` read protections, the `denyRead` rules, and the automatic write-deny on `settings.json` at every scope, while sandboxed commands stay auto-allowed. The vendor documentation flags that combination as self-escalating: a command can write a shell startup file or `settings.json` that widens its own access on the next run. The key is honored from user, managed, and `--settings` sources but not from project or local settings, and managed settings can reserve it exclusively by configuring `filesystem` or listing any `sandbox.credentials.files` entry. The assessor verifies the resolved value of this key, because the presence of `sandbox.enabled` says nothing about it.

One property of this plane has no settings key, and so gets no row. **A sandbox constrains only what runs after it initializes, and a harness can read workspace configuration before that point.** [[gemini-cli-workspace-trust-rce|GHSA-wpqr-6v78-jr5g]] is the case on the public record: headless Gemini CLI trusted the workspace folder for configuration and environment loading and executed from an attacker-supplied `.gemini/` tree before its sandbox initialized ([Novee Security, 2026-04-30](https://novee.security/blog/google-gemini-cli-rce-vulnerability-cvss-10-critical-security-advisory/)). *Whether* workspace trust is granted can be a key — Gemini CLI's `GEMINI_TRUST_WORKSPACE` is one since 0.39.1. *When* the isolation boundary is established relative to that read is a property of the harness's startup sequence, which no vendor documentation states. Every row above is assessable by resolving a settings value; this one is an open question to put to a vendor rather than a control to score.
...
Masking carries preconditions a configuration can miss. It is honored only from user, managed, and `--settings` sources — masking entries in a repository's `settings.json` are ignored — and every `injectHosts` entry must itself be covered by `allowedDomains`. Without `tlsTerminate` the sentinel reaches the server unchanged and authentication fails rather than leaking, which is the right failure direction but means the control is either working or visibly broken, never silently partial.
...
Nine rows anchor to a CMM domain but to no RA capability. They fall into three groups, and each group places its gap differently.
...
**Shape-specific artifacts.** Pinning a session to a known Anthropic organization, excluding untrusted input from CI triggers, and the first-party analytics APIs are artifacts of how this particular product is licensed, triggered, and instrumented. They belong in a catalog and not in a reference architecture.

## Method

The deployment order below follows the control ranking rather than the plane order.
...
**The catalog's rows carry CMM coordinates for D2 through D8 and none for D1 or D9, and one D1 criterion reads its evidence off them.** [[agentic-ai-security-cmm-d1-governance|D1]] L3 grades the harness configuration for this shape: managed policy that local configuration cannot override or, where the harness offers a lock, extend, and every change to the configuration tree held under review. The first half comes from the managed-permission-policy row in the [Control plane](#control-plane) together with the lock rows beside it, because array keys merge local entries unless a managed-only key is set, and `excludedCommands` stays extendable with no lock. The second half comes from step 6 of the [Method](#method), a method step with no row of its own, which puts the repository's configuration tree under source control; the strict-customization lock in the MCP row blocks the user- and project-sourced skills, agents, hooks and MCP servers that source control does not reach, leaving plugin delivery as the route the review has to cover. The rest of the D1 ladder and the whole [[agentic-ai-security-cmm-d9-operations|D9]] ladder still have no row on this page to cite, so an assessor scores a coding deployment's remaining governance and human-factors evidence from those ladders directly.

[[cmm-stress-test-canadian-fi-google-2026-09|A Canadian-FI stress test against Google Cloud]] found three single-stack defects in this catalog against that persona: no MCP-allowlist row, no row for cloud-routed inference, and a wikilink that named this page's product as the vulnerability-discovery tool rather than the harness. The MCP-allowlist and cloud-routing rows above and the corrected link answer those three findings; the D9 coordinate gap, the D1 coordinates beyond the harness-configuration criterion, and the D8 fleet-inventory dimension the stress test also found remain open, recorded in [[cmm-known-limitations|CMM Known Limitations]] items 10 and 27.
...
[^ccsandbox]: [Anthropic — Claude Code sandboxing](https://code.claude.com/docs/en/sandboxing), fetched 2026-09-18. Security limitations: the built-in proxy makes its allow decision from the client-supplied hostname and by default does not terminate or inspect TLS, so code running inside the sandbox can use domain fronting or similar techniques to reach hosts outside the allowlist, and stronger TLS-aware network isolation is named as an active area of development. The experimental `tlsTerminate` setting, available in Claude Code v2.1.199 and later, terminates TLS at the proxy for masked-credential substitution and adds no content filtering; a threat model requiring stronger guarantees is directed to a custom proxy that terminates TLS, inspects traffic, and carries its CA certificate inside the sandbox.
