...
sources:
  ...
  - "https://docs.cloud.google.com/iam/docs/release-notes#April_22_2026"
  - "https://docs.cloud.google.com/gemini-enterprise-agent-platform/release-notes#August_31_2026"
  - "https://docs.cloud.google.com/vpc-service-controls/docs/release-notes#June_29_2026"
verified: 2026-09-24
verified_against: []
verified_findings: 0
verified_note: "Read whole; live Google pages re-read 2026-09-24 (four GA release-note entries, UAP note, VPC-SC products, Model Armor notes, overview, regions and residency, Agent Platform residency, Workspace access); lead scope, dating, D6 reconciliation and armorrn fixed; none open."
---

# Google Cloud Agentic Security Profile

Google Cloud, read alone against the six planes of the [[agentic-ai-security-reference-architecture|Agentic AI Security Reference Architecture]] and the nine domains of the [[agentic-ai-security-cmm-2026|Agentic AI Security Capability Maturity Model]], leaves three plane requirements without a native control, each an absence on the Google page that governs it: no control that narrows a Workspace answer at the time it is composed, no Model Armor integration for Workspace traffic, and no detector for agent behavioral drift. Its agent decision point and its egress perimeter do not compose, a Canadian Agent Platform deployment holds two regions from the first day, and the Workspace assistant has no Canadian region at all. The reading answers the May regulated-FI stress test's open closure condition: "a **single-stack reading** of the RA per major platform (Microsoft-only, AWS-only, GCP-only) that names where the platform has no native control and an off-stack component is unavoidable" ([[agentic-cmm-regulated-fi-stress-test|Agentic AI CMM: Regulated-FI Stress Test]]). The Microsoft-only and AWS-only readings stay open.

The worked example is Gemini for Google Workspace, an in-suite assistant holding tools over a whole tenant's mail, files and calendar, which [[cmm-stress-test-canadian-fi-google-2026-09|CMM Stress Test: Canadian FI on Google Cloud]] scored on 2026-09-15. Product status is a documentation snapshot of 2026-09-16, the four GA dates in the status column come from Google's release notes as read on 2026-09-24, and Google's pages name its generative-AI surface Gemini Enterprise Agent Platform, formerly Vertex AI.
...
The assistant reads private data, ingests untrusted content on every summarization of an inbox or a calendar, and writes and shares, so all three legs of the [[lethal-trifecta|Lethal Trifecta]] are present. [[azure-rag-chatbot-security-profile|Azure-Native RAG Chatbot Security Profile (Copilot Studio)]], the vault's other deployment-shape profile, covers a closed-corpus bot whose broken trifecta lowers the required level across five domains. That reduction does not transfer here, and no plane falls out of scope.

## The control profile per plane and domain

**The second column states a gap**: the level Google's documentation evidences, then the level this shape needs. A right-sizing table writes the same arrow as a two-level target range, and [[cmm-vocabulary-and-notation|CMM Vocabulary and Notation]] separates the two readings.

| Plane / Domain | Evidenceable → needed | Google control | Status as documented | The one thing that matters |
|---|---|---|---|---|
| **Identity ([[agentic-ai-security-cmm-d2-identity\|D2]])** | L2 → L3 | Agent Identity: a SPIFFE ID ([[spiffe\|SPIFFE / SPIRE]]) per agent, a 24-hour X.509 certificate, and access tokens bound to it[^agentid] | GA since 2026-04-22 in the IAM release notes; the overview states no launch stage[^agentidga][^agentid] | An agent on Agent Runtime gets a first-class principal; the Workspace assistant acts as the employee and has none |
| **Control ([[agentic-ai-security-cmm-d3-control-least-agency\|D3]])** | L1 to L2 → L3 to L4 | IAM Unified Access Policies at Agent Gateway, evaluated by Identity-Aware Proxy, with CEL conditions and a DRY_RUN mode before ENFORCE[^uap] | GA since 2026-08-31 in the Agent Platform release notes; the overview states no launch stage[^uapga][^uap] | A declarative decision point exists and excludes the egress perimeter (below) |
...
| **Observability ([[agentic-ai-security-cmm-d7-observability\|D7]])** | L2 → L3 | OpenTelemetry GenAI semantic conventions into Cloud Trace and Application Monitoring; Google SecOps; Agent Platform Threat Detection[^otel][^aptd] | Agent Observability GA since 2026-06-18[^obsga], with no launch stage on the OTel page[^otel]; Agent Platform Threat Detection Preview[^aptd] | The threat detector reads host and control-plane compromise, and grades no agent behaviour |
...
The first column reproduces the September scoring of this shape, level for level, and adds no level of its own. Its needed half came from each domain's right-sizing table when none of the nine carried a row for this shape, so it differs from the in-suite row the core shape table now carries, L3 across all, L4 in D6 and D9, and L2 → L3 in D8: the needed half reaches a level above the row in D1, D3 and D4 and a level below it in D6, and sits a level below it in D9. The reference architecture's shape table carries the same row as per-plane controls. The core row is the target an assessor scores against, and the needed half stays as the September reading of the tables as they then stood.

## The four controls the customer holds

[[google|Google]] documents four controls the customer holds, and the list ends there: per-organizational-unit enablement, data-loss-prevention rules bounding what the assistant reaches, an administrative control centre with a usage review, and a stated training restriction over customer data.[^gemou][^gemdlp][^gemaicc][^privacyhub] The vendor's own account describes four layers against indirect prompt injection ([[securing-workspace-genai-at-google-talk|Securing Workspace GenAI at Google]]):

- visible-content-only input
- structural hierarchy in the prompt
- deterministic orchestration sandboxing that restricts downstream capability by data origin
- output hardening that scrubs image tags and ungrounded links

Three of the four are unevidenceable from the customer's side, which holds D3, D4 and D5 at L1 to L2 for this shape.

## Planes with no Google-native control

On three planes Google ships an instrument that stops short of what the plane requires, and on each the shortfall is in the control rather than in its maturity. [[google-saif|Google SAIF: Secure AI Framework]] closes none of the three, because it names control categories without specifying controls, thresholds or test procedures.

### Answer-time entitlement enforcement

Google documents permission inheritance and one wholesale admin lever, and nothing between them. "Gemini has the same access to Workspace data as you do", and an administrator can restrict access to Gemini entirely or to some or all Workspace data.[^gemaccess] Two narrowings exist, each incidental to its own feature: a Drive file whose owner blocks download, copy and print stays out of reach, and delegated Gmail access does not carry over.[^gemaccess] That page documents no control that narrows an answer at the time the answer is composed, which is the Data plane's load-bearing requirement and the control the Azure profile meets with Entra-authenticated per-user trimming, with Restricted SharePoint Search as its stopgap. [[oversharing-controls|Oversharing Controls for AI Search]] owns the failure mode, and the assistant surfaces it as [[inference-exposure|Inference Exposure (and Retrieval Exposure)]]: an answer composed from files the employee may open and never would.

Google Cloud DSPM leaves the gap open. Its supported asset types are BigQuery datasets and tables, Cloud Storage buckets, and Agent Platform models, datasets, feature stores and metadata stores, with Cloud Storage objects added at Preview, and `Drive` does not appear on the page. The page states no launch stage of its own, three of its controls carry Preview markers, and the service shuts down on 2027-02-01.[^dspm] A buyer who needs answer-time narrowing over Workspace content buys it off-stack or accepts corpus permissions as the only control.
...
Google's agent-observability guidance names drift as a risk ("AI agents can drift, hallucinate, and regress silently") and documents no drift detector, anomaly rule or baseline to act on it.[^drift] Agent Platform Threat Detection, under a page-level Preview banner, detects the execution of malicious binaries or scripts, container escapes, reverse shells and attack tools in the agent's environment, plus control-plane activity from Event Threat Detection, among it data exfiltration attempts, excessive permission denials and suspicious token generation.[^aptd] Those detectors read host and control-plane compromise and grade no agent behaviour, so an agent talked into misusing the access it legitimately holds registers only where its actions surface as one of those events, and never as a departure from how that agent normally behaves.

## The agent decision point and the egress perimeter do not compose

IAM Unified Access Policies adjudicates agent-to-resource calls declaratively: policies bind to the project holding the Agent Gateway instances, each rule carries both an allow effect and a deny effect, conditions are written in CEL, a Principal Access Boundary can be set on the agent identity, and DRY_RUN logs disallowed agentic communications before ENFORCE blocks them.[^uap] The page opens with its own limit, "This feature does not support VPC Service Controls", and the note still stood on 2026-09-24, after the feature reached general availability.[^uap][^uapga] Agent Gateway does not appear on the VPC Service Controls supported-products page, where Model Armor, Agent Runtime, Secure Web Proxy and Sensitive Data Protection each carry a GA integration.[^vpcsc]

A single-stack buyer picks one. Adjudicating agent-to-resource calls at the gateway puts the decision point outside the service perimeter that protects the data plane around it, and drawing the perimeter first leaves the agent-to-agent and agent-to-MCP paths to IAM allow and deny policies without the gateway's rule model. The RA's Control and Egress planes assume both controls run together, and on this platform they do not.

That choice exists only where the customer runs the agent. The Workspace assistant, the tools it calls and the path between them all sit inside the vendor, so no perimeter, proxy or gateway the customer buys can take a position in that path. On the Egress plane for this shape the closure condition's question therefore runs the other way: [[cmm-stress-test-canadian-fi-google-2026-09|CMM Stress Test: Canadian FI on Google Cloud]] holds D5 at L1 to L2 and records the domain among those the bank cannot remediate.
...
The guardrail therefore sits in Toronto beside the agent runtime it screens, and the model answering that agent is served from Montréal or from outside Canada. The split is a property of the platform's coverage rather than a misconfiguration a buyer can tidy away.

Seven of the twenty-seven Google-model rows on the residency table carry a `Canada (northamerica-northeast1)` commitment: Gemini 3.5 Flash, Gemini 2.5 Flash at 128k and 1M, Gemini 2.5 Pro at 64k and 1M, and the two text-embedding models. No 3.x flagship above 3.5 Flash carries one, and neither does any tuning row.[^residency] Partner models sit on a separate table with no Canada column at all, so the coding shape described in [[securing-agentic-coding|Securing Agentic Coding]] has no Canadian residency option to configure.[^residency] ML processing follows the endpoint the caller uses,[^residency] and the locations page states that endpoints guarantee neither data residency nor in-region ML processing.[^locations]

### The guardrail trade

Toronto is a limited-support Model Armor region, and the reduction is in filters rather than in residency. A template with data-residency compliance enabled keeps the responsible-AI filters, Sensitive Data Protection, and [[prompt-injection|Prompt Injection]] and jailbreak detection, and loses malicious URL detection, multi-language detection, CSAM support, image support and antivirus scanning, "because they rely on services that might process data outside the jurisdiction of your chosen Model Armor region".[^featreg] That template's residency row reads `Yes / Yes / Yes` for data at rest, in use and in transit.[^maresidency] Floor settings on the Agent Platform integration restore every filter and weaken the residency guarantee: Canada's floor-settings row reads `Yes / No / No`, and the page defines that `No` as "the data might be processed or transmitted outside of the jurisdiction".[^maresidency] At-rest residency survives either choice, so the trade is over processing and transit.

**Malicious URL detection is the filter a residency-enforced Toronto template gives up over the exfiltration route this shape's recorded attacks used.** [[geminijack-gemini-enterprise-injection|GeminiJack Gemini Enterprise Zero-Click Injection]] exfiltrated mail, calendar and document content through an auto-loading `<img>` tag in the rendered answer, and [[echoleak-copilot-zero-click|EchoLeak Zero-Click Copilot Exfiltration]] carried data out of another vendor's assistant by routing an image request through an allowlisted first-party link-preview endpoint. Each exfiltration leg is an outbound fetch the client performs as it renders markup, and malicious URL detection is the Model Armor filter that reads the URLs in a response. Model Armor's image screening sits off that route: it screens images supplied in prompts and responses by visual scanning and optical character recognition, so it inspects what an image carries and never a URL the client fetches.[^armor] Google's own pages then disagree about Toronto. The feature-availability table lists image support among the features a residency-enforced Toronto template loses, while the overview supports image screening in the `us` and `eu` multi-regions only, which would leave that template no image screening to lose.[^featreg][^armor] EchoLeak also bounds the filter that does apply, because its egress URL belonged to the vendor's own allowlisted service and a reputation check clears such a URL.

Workspace output hardening sits on that route, and it is not Model Armor. The vendor's account of it names markdown scrubbing of image tags and link protocols, Safe Browsing filtering of every URL, and removal of ungrounded links the model produced without a source in its input ([[securing-workspace-genai-at-google-talk|Securing Workspace GenAI at Google]]). No region gates that layer and no customer configures it. A Canadian institution that enforces residency on the guardrail therefore gives up the response-side URL filter over the route [[indirect-prompt-injection|Indirect Prompt Injection]] against this shape has used, and what still covers that route is a vendor-side layer the institution cannot evidence. Floor settings restore the filter and accept processing outside the jurisdiction. Both horns are configurations, and the choice belongs in the third-party-risk file that [[agentic-ai-security-cmm-crosswalk-canada-fi|CMM: Canadian Regulated-Finance Crosswalk]] maps to OSFI Guideline B-10.
...
- **The processing half of Workspace data-regions coverage varies by edition.** Gemini prompts and responses are covered data at rest and during processing, and retention for Workspace prompts and responses runs 90 days to indefinite as determined by admins.[^dataregions][^privacyhub]
...
[^agentidga]: [Google Cloud — IAM release notes](https://docs.cloud.google.com/iam/docs/release-notes#April_22_2026), fetched 2026-09-24. The 2026-04-22 entry reads "Agent Identity is generally available (GA)"; the [2026-08-22 entry](https://docs.cloud.google.com/iam/docs/release-notes#August_22_2026) makes the Agent Identity auth manager and the Agent Identity APIs generally available.
[^uap]: [Google Cloud — IAM Access policies overview](https://docs.cloud.google.com/gemini-enterprise-agent-platform/govern/policies/iam-overview-uap), fetched 2026-09-16 and re-read 2026-09-24. Declarative agent-action authorization enforced at Agent Gateway through Identity-Aware Proxy; the page's opening note states "This feature does not support VPC Service Controls" and no launch stage, on both dates.
[^uapga]: [Google Cloud — Gemini Enterprise Agent Platform release notes](https://docs.cloud.google.com/gemini-enterprise-agent-platform/release-notes#August_31_2026), fetched 2026-09-24. The 2026-08-31 entry reads "IAM Unified Access Policies (Access policies) for Gemini Enterprise Agent Platform are generally available (GA)"; the [2026-06-18 entry](https://docs.cloud.google.com/gemini-enterprise-agent-platform/release-notes#June_18_2026) is headed "Agent Gateway in General Availability".
...
[^armorrn]: [Google Cloud — Model Armor release notes](https://docs.cloud.google.com/model-armor/release-notes), fetched 2026-09-16 and re-read 2026-09-24. Dated entries announcing GA for individual Model Armor features and integrations; the [2025-02-03 entry](https://docs.cloud.google.com/model-armor/release-notes#February_03_2025) introducing the service states no launch stage, and no later entry gives the service one.
...
[^vpcscrn]: [Google Cloud — VPC Service Controls release notes, June 29, 2026](https://docs.cloud.google.com/vpc-service-controls/docs/release-notes#June_29_2026), fetched 2026-09-24. The entry states that using agent identities, and SPIFFE formats for third-party identities, in ingress and egress rules "is generally available".
...
[^obsga]: [Google Cloud — Gemini Enterprise Agent Platform release notes, June 18, 2026](https://docs.cloud.google.com/gemini-enterprise-agent-platform/release-notes#June_18_2026), fetched 2026-09-24. The entry reads "Agent Observability is generally available (GA)" and turns OpenTelemetry tracing on by default for newly deployed Agent Development Kit agents on Agent Engine.
