...
Per-function deep-dive for the **Exposure & VulnOps** row of the [[agentic-soc-reference-architecture|Agentic SOC Reference Architecture]]. The function runs continuous exposure and vulnerability discovery, plus remediation, across the whole estate — own code, AI-generated and vibe-coded applications, third-party libraries, container images, and the cloud control plane. It is the SOC's seam with DevSecOps: where build-time security hands off to operate-and-monitor, and where production exposure is owned. The function is the operational home of [[vulnops|VulnOps]] — discovery and remediation staffed and automated like DevOps rather than run as a quarterly audit cycle.
...
The driver is the time-to-exploit collapse. The [[zero-day-clock|Zero Day Clock]] records the median time from CVE disclosure to first observed exploitation falling from 771 days in 2018 to zero-day in 2025–2026, where the exploit arrives on or before the advisory.[^zdc] Periodic vulnerability management — a quarterly pen test plus patch-as-CVE-arrives — is structurally outmatched against continuous AI-driven discovery, and the estate is now too large and too fast-changing for humans to inventory or patch in time.
...
Two changes to the function's shape follow from the [[openai-hugging-face-agent-incident|OpenAI–Hugging Face agent incident]].[^bh] The first is that the discovery side acquires a standing red-team component. Autonomous agents found and chained two zero-days in a production enterprise repository manager and two more in a production dataset platform, with no prior knowledge of any of them. That makes continuous agentic red teaming a permanent function of this row rather than a periodic engagement on an audit calendar. It belongs to the discovery agent's remit — read-heavy, reversible against a staging surface, gated on the same D1 data — and its cadence is set by adversary capability rather than by an audit cycle. The second is that the L3 and L4 rows above describe the remediation leg the source names as the actual gap; it is treated as a failure mode below.
...
| AI-assisted code-audit / discovery | [[openant\|OpenAnt]] (OSS), [[codex-security\|Codex Security]] (OpenAI), [[claude-code-security\|Claude Code Security]] (Anthropic), [[codemender\|CodeMender]] (Google Cloud); vendor-internal [[big-sleep\|Big Sleep]]; [[mythos\|Mythos]]-class models via Glasswing partners | Preview / private-preview for most; CodeMender in managed preview since [[google-cloud-codemender-preview\|July 2026]]; Big Sleep vendor-internal; Mythos is preview-only, no GA planned |
...
| Exploitability triage | Severity/confidence scoring, deduplication, reachability and applicability analysis as queue stages; [[frontier-ai-for-vuln-discovery#The validation discipline\|adversarial-reflexion-style false-positive control]]; sandboxed proof-of-concept exploit construction as a gate before patching ([[codemender\|CodeMender]] verify stage) | Pattern-level; the load-bearing scarce-resource discipline. Now packaged in commercial preview, without published false-positive data |
...
- **Coverage blind spots.** A patch or isolation acts on a defensible picture only if the asset and exposure inventory is real. An un-inventoried asset, a silent telemetry source, or an unresolved dependency is exposure the function cannot see. Bounded by **D1**: measured coverage against the threat model, not assumed completeness. [[mythos-ready-security-program|Mythos-ready PA 7]] makes the point — you cannot patch, segment, or defend what you do not know exists.
...
- **The un-automated remediation leg.** The failure mode most likely to bound this function is not a bad automated patch but the absence of one. Where discovery runs at L3 and rollout still crosses a human change-approval queue, every gain in finding rate lands in a backlog, and the function's measured output is a longer queue at an unchanged exposure window. The loop closes only through identify, propose patch, roll out, and **roll back** on an availability regression, with the reversal engineered into the pipeline; automated rollout without automated rollback turns a wrong fix into an outage and forces the human gate back in one stage later.[^bh] Bounded by treating rollback capability as a prerequisite for raising remediation autonomy, not as a recovery afterthought, and by measuring the finding-to-deployed-fix interval rather than the finding count.
- **Upstream disclosure as an uncosted action.** The discovery agent's scope covers third-party libraries and container images, so a share of its findings are flaws in software the organization does not own and cannot patch. The function's only lever there is upstream disclosure, and one academic measurement puts that lever's sign in question: on embedded devices, at human pace and with no agents involved, disclosing a vulnerability endangered roughly three times as many devices as it secured, because a disclosed flaw carries [[vulnerability-properties|properties]] that transfer to targets outside the advisory's scope.[^asu-keynote] The finding-to-deployed-fix interval this function measures does not register that cost — it stops at the organization's own estate. Bounded by treating an upstream disclosure decision as a governed action with a stated exposure window, scored under **D8 People & Governance** alongside the remediation-authority tiers, rather than as an automatic consequence of a confirmed finding. No source on the wiki offers a disclosure model calibrated for machine-scale discovery, so this is a named gap rather than a control.
...
|---|---|---|
| **Solo / small** | Discovery L2–L3, remediation L1–L2 | Near or below the [[cyber-poverty-line\|cyber poverty line]]. Borrow exposure coverage and triage via an MSSP/MDR/ISAC; AI lowers the floor so continuous scanning and AI-assisted triage are reachable without a built-out program. Remediation stays human-approved — a small team's blast-radius controls are thin, so auto-remediation is not yet earned |
