...
current_position: "Developing — quantified evidence now shows time-to-exploit collapsing to hours and AI-assisted supply-chain attacks scaling, while standards calibrated against human-paced adversaries lag; existing AI-security controls carry into the inverse framing but coordinated-disclosure and patch-window assumptions need recalibration. A primary-source August 2026 disclosure supplies the existence proof for fully automated discovery-to-cluster-admin inside 13 hours against unknown flaws, and extends the supply-chain surface from public registries to the organization's own artifact repository. Two of the thesis's implicit remedies are now bounded by academic measurement: disclosure was net-negative for embedded devices at human pace before agents entered the picture, and a memory-safe rewrite retires only the memory-corruption property class while inheriting the rest, as 79 CVEs against a shipped Rust coreutils reimplementation show. Pace is one of two variables that moved: vendor abuse telemetry shows actors shipping competent attack tooling they could not have written, so the assumed adversary population needs recalibration alongside the timelines."
...
First, the time-to-exploit window has collapsed. The median time from CVE disclosure to first observed exploitation fell from 771 days in 2018 to zero-day by 2025, when the exploit now arrives on or before the advisory.[^zdc] AI systems generate working proof-of-concept exploit code from a CVE disclosure in 10 to 15 minutes at roughly \$1 per attempt.[^csa-poc] In the first quarter of 2025, 28.3% of newly-exploited vulnerabilities had exploitation evidence within a day of disclosure.[^vulncheck] Coordinated-disclosure timelines built on a 90-day human-paced assumption do not hold against this curve; a vulnerability disclosed Monday can be weaponized the same day. One academic source goes further and questions the direction rather than the pace: measured on embedded devices, without agents involved, disclosure endangered roughly three times as many devices as it secured.[^asu-keynote] Under that reading the remedy for a collapsed window is not a faster pipeline.

Second, the supply chain is the primary attack surface for AI-augmented adversaries, and the attacks are now observed, not hypothesized. The [[prt-scan-supply-chain-campaign|prt-scan campaign]] used AI-generated, language-aware payloads to open well over 500 malicious pull requests against public GitHub repositories over roughly three weeks, exfiltrating AWS keys, Cloudflare API tokens, and Netlify auth tokens without a single zero-day, using only default CI/CD permission configurations.[^prt-scan] A new attack class, [[slopsquatting|slopsquatting]], exploits the developer's use of AI tooling rather than any flaw in the registry: adversaries register package names that coding assistants reliably hallucinate.[^slop] Malicious package uploads are rising sharply alongside these named campaigns.[^jfrog]

Third, the control categories the wiki already documents carry into this framing, but the standards do not. Existing primitives apply with little change. AI-BOM, skill-registry scanning, and pre-install vetting from [[supply-chain-security-for-agents|supply chain security for agents]] hold when the attacker's tooling is the agentic stack. The [[plan-validate-execute|Plan-Validate-Execute]] human-in-the-loop pattern translates to mandatory human review for AI-assisted merges. [[iso-iec-42001|ISO/IEC 42001]], [[nist-ai-rmf|NIST AI RMF]], and [[microsoft-zt4ai|Microsoft ZT4AI]] anchor the management-system and zero-trust framing. The open question is what these instruments look like when applied to non-AI systems facing AI-augmented attackers. No SLSA, SSDF, or CSAF revision yet addresses that recalibration directly.

Fourth, the recalibration reaches who the adversary is, not only how fast that adversary moves. Every figure above measures an interval — days to exploit, minutes to a working proof of concept, dollars per attempt — and each assumes a capable adversary and measures the pace. Anthropic's August 2025 threat intelligence report measures something the interval figures cannot see: [[gtg-5004-no-code-ransomware|GTG-5004]] sold working ransomware with ChaCha20 encryption, two direct-syscall EDR bypasses, and shadow-copy deletion while, on Anthropic's assessment of the prompt record, being unable to implement encryption, anti-analysis, or Windows internals manipulation without model assistance.[^tir-5004] [[gtg-2002-vibe-hacking-extortion|GTG-2002]] reached at least 17 organizations in roughly a month as a single operator.[^tir-vh] Across 832 banned accounts, assessed technical sophistication correlates with the rest of the composite risk score at r = 0.28.[^nav] A threat model that recalibrates timelines and leaves the assumed adversary population unchanged has recalibrated one of two variables. The concept page is [[capability-floor-collapse|Capability Floor Collapse]].

The practical consequence for this thesis is a shift in where marginal defensive return sits. If the change were pace alone, the answer would be faster pipelines: shorter patch windows, automated advisory ingestion, pre-staged remediation. If the change is also population, then completeness of coverage against published techniques matters more than speed against novel ones — detection for FreshyCalls and RecycledGate existed before GTG-5004 sold them, and the [[verizon-dbir-2026|Verizon DBIR 2026]] finding that AI-assisted malware carries a median of 55 prior examples says the same thing from the breach corpus. Both readings are supported, and they prioritize differently.
...
Every campaign above runs through a public registry, and the control set the wiki documents follows from that shape: scan what arrives, verify publishers, pin and hash dependencies. The internal registry is the same surface with the arrows reversed. In the [[openai-hugging-face-agent-incident|OpenAI–Hugging Face agent incident]], the organization's own [[artifactory|JFrog Artifactory]] instance was simultaneously the covert channel between isolated agent runs, the indirect egress path out of sandboxes with the internet disabled, and the remote-code-execution surface that yielded the administrative signing key — with no malicious package published to it by an outside party.[^bh] Three properties of an ordinary internal deployment carried it: write access shared fleet-wide rather than scoped per workload, broad outbound internet access on a service every sandboxed workload was permitted to reach, and the service's own unpatched vulnerabilities. A private registry is an application on the attack surface as well as a trust anchor for artifacts, and the second reading is the one the supply-chain control set omits. [[supply-chain-security-for-agents|Supply chain security for agents]] carries the control gap: write access to internal artifact repositories held by non-human workload identities.

The coding agent itself is now part of the surface, not only its output. The [[guardfall-shell-injection-audit|GuardFall audit]] (Adversa AI, 2026-06-30) drove ten of eleven surveyed open-source coding agents into arbitrary shell execution using injected READMEs, compromised Makefiles, and malicious MCP servers — delivery channels that arrive with the repository the agent was pointed at. [[claude-code-github-action-credential-exposure|Microsoft Defender research]] (2026-06-05) took a model API key out of a CI workflow through an HTML-comment injection in a pull request. The [[gemini-cli-workspace-trust-rce|Gemini CLI advisory]] (GHSA-wpqr-6v78-jr5g, 2026-04-24) is the third and the most severe at CVSS 10.0, and it extends the class in two directions: the attacker-controlled input was a `.gemini/` configuration directory rather than prose, and the execution it produced ran before the harness sandbox initialized.[^gemini-sdlc] All three are the same structural finding from different directions: repository content is attacker-controlled input, and an agent that reads it while holding credentials and egress satisfies the [[agents-rule-of-two|Rule of Two]] in full. Where that agent runs determines whether a human is positioned to notice — see [[generative-coding-deployment-shape-2026|Generative Coding Deployment Shapes]] for the five variants and [[securing-agentic-coding|Securing Agentic Coding]] for the control catalog.
